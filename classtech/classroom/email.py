...
from classtech.settings import EMAIL_HOST_USER
from classroom.models import Submissions, Assignments, Classrooms, Students, CustomUser
...
def submission_marks_mail(submission_id, teacher_id, marks):
    ...
    message = 'Dear, {}, your submission for the assignment {} has been graded {} by Prof. {}'.format(
        student_username, assignment_name, marks, teacher_name)
...
def assignment_post_mail(classroom_id, assignment_id):
    ...
    message = 'Dear Students, {} assignment has been posted to {}. Due date of the assignment is {}'.format(
...
def submission_done_mail(assignment_id, user, submitted_file):
    ...
    assignment = Assignments.objects.get(pk=assignment_id)
    assignment_name = assignment.assignment_name
    message = 'Dear Student {}, you have made a submission {} for the assignment {} on {} .'.format(
        user.username, submitted_file.name, assignment_name, datetime.now())
