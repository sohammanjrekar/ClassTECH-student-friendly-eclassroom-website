...
def ajax(request):
    ...
    return render(request, 'attendence/ajax.html', context)
    ...
    while True:
        ...
        if process_this_frame:
            ...
            face_encodings = face_recognition.face_encodings(
                rgb_small_frame, model='large', known_face_locations=face_locations)
            face_names = []
...
def profiles(request):
    ...
    return render(request, 'attendence/profiles.html', context)
...
def details(request):
    ...
    return render(request, 'attendence/details.html', context)
...
def add_profile(request):
    ...
    try:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                encoding_image(str(int(request.POST.get("phone"))),
                               request.FILES["image"])
                form.save()
    ...
    context = {'form': form}
    return render(request, 'attendence/add_profile.html', context)
    ...
    try:
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                try:
                    encoding_image(
                        str(int(request.POST.get("phone"))), request.FILES["image"])
...
def delete_profile(request, id):
    ...
    del_dict[profile.pk] = {'first_name': profile.first_name, 'last_name': profile.last_name, 'date': profile.date,
                            'hostelname': profile.hostelname, 'hosteltype': profile.hosteltype, 'roomno': profile.roomno, 'phone': profile.phone}
...
def encoding_image(name, image):
    ...
    try:
        ...
        person_face_encoding = face_recognition.face_encodings(
            image_of_person, num_jitters=100)[0]
...
def signin(request):
    ...
    return render(request, 'attendence/signin.html')
...
def home(request):
    return render(request, 'attendence/open.html')
...
def signup(request):
    if request.method == "POST":
        ...
        if User.objects.filter(username=username).exists():
            messages.error(
                request, "User already exist! try other username....")
            return redirect('home')
    ...
    return render(request, "attendence/signup.html")
...
def download(request):
    ...
    if request.method == 'GET':
        ...
        if os.path.getsize("media/picklefiles/attendance.pickle") > 0:
            ...
            attendance = {}
        if os.path.getsize("media/picklefiles/delete_user.pickle") > 0:
            del_list = pickle.loads(
                open('media/picklefiles/delete_user.pickle', "rb").read())
        else:
            del_list = {}
        ...
        file = "media/attendance_documents/" + date + \
            "_"+present+"_"+hostel+"_"+"file.csv"
        with open(file, 'w') as f:
            f.writelines(
                f'first_name,last_name,phone_number,parent_phone,hostelname,hosteltype,roomno')
            f.close()
        ...
        # print(dt)
        dt.to_csv("media/attendance_documents/"+date+"_" +
                  present+"_"+hostel+"_"+'file.csv', index=False)
        # return HttpResponse(dt.to_html())

        print(dt)
        file_server = pathlib.Path(
            "media/attendance_documents/"+date+"_"+present+"_"+hostel+"_"+'file.csv')
        if not file_server.exists():
            ...
            if option == 'download':
                ...
                response['Content-Disposition'] = 'inline; filename=' + \
                    date+'"_"'+present+'"_"'+hostel+"_"+hosteltype+'file.csv'
                # print(123)
                return response
            ...
            return render(request, 'attendence/table.html', context)
    return redirect('index')


@login_required
def month_attendance(request):
    return render(request, 'attendence/attendance.html')


@login_required
def day_attendance(request):
    return render(request, 'attendence/day_attendance.html')
...
def manual_checking(request):
    if request.method == 'GET':
        ...
        try:
            ...
            return render(request, 'manul_attendance.html', context)
        except:
            # print('sorry')
...
def pickel_attendance(dte):
    ...
    try:
        ...
        for profile in profiles:
            ...
            d[profile.pk] = {'first_name': profile.first_name, 'last_name': profile.last_name, 'date': profile.date, 'hostelname': profile.hostelname,
                             'hosteltype': profile.hosteltype, 'roomno': profile.roomno, 'phone': profile.phone, 'parentphone': profile.parentphone}
    ...
    try:
        if request.method == 'GET':
            ...
            date1 = datetime.strptime(
                request.GET.get("date1", ""), "%Y-%m-%d").date()
            date2 = datetime.strptime(
                request.GET.get("date2", ""), "%Y-%m-%d").date()
            present = request.GET.get("present", "")
            hostel = request.GET["hostel"]
            hosteltype = request.GET["hosteltype"]
            option = request.GET["option"]
            ...
            for i in l:
                if i in attendance.keys():
                    ...
                    r = list(set(r))
            print(r)
            file = "media/attendance_documents/" + date1.strftime("%Y-%m-%d")+"to"+date2.strftime(
                "%Y-%m-%d")+"_"+present+"_"+hostel+"_"+hosteltype+"file.csv"
            with open(file, 'w') as f:
                f.writelines('first_name,last_name,phone,parent_phone,hostelname,hosteltype,roomno,' +
                             ",".join(l)+',total_present_days,total_absent_days')
                f.close()
            for student in r:
                s = []
                u = []
...
def hostelreport(request):
    ...
    present = att[1]
    att_db = att[2][0]
    hostellist_absent = {'cvr': 0, 'vvk': 0,
                         'asr': 0, 'vsr': 0, 'dnr': 0, 'sac': 0}
    hostellist_present = {'cvr': 0, 'vvk': 0,
                          'asr': 0, 'vsr': 0, 'dnr': 0, 'sac': 0}
    for i in absent:
        att_user = att_db[i]
    ...
    if option == 'download':
        response = FileResponse(
            dt.to_csv(), content_type='application/force-download')
        response['Content-Disposition'] = 'inline; filename=' + \
            day+'_hostel report.csv'
        return response
...
def studentreport(request):
    ...
    if option == 'download':
        response = FileResponse(
            df.to_csv(), content_type='application/force-download')
        response['Content-Disposition'] = 'inline; filename=' + \
            str(phone)+'_hostel report.csv'
        return response
